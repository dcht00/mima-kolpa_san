...
$.fn.editable = function(options){
	...
	options.toEditable = function(el){
		$this = $(this);
		...
		if(opts.type == 'title' && !$this.parents('article').hasClass('open')) {
			//alert('opened');
			return false;
		}
		//return false;
		...
		// Submit Event
		var buttontype = (opts.type == 'title' ? 'click' : 'mouseup');
		if(opts.submit){
			$('<button class="editableapplybutton" />').appendTo($this)
						.html(opts.submit)
						.one(buttontype,function(){opts.toNonEditable($(this).parent(),true)});
		}else
		...
		// Cancel Event
		if(opts.cancel)
			$('<button class="editablecancelbutton"/>').appendTo($this)
						.html(opts.cancel)
						.one(buttontype,function(){opts.toNonEditable($(this).parent(),false)});
		// Call User Function
